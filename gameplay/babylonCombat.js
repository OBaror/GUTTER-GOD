...
import { getStamina }                   from './babylonTraversal.js';
import { getEngine }                    from '../engine/babylon/runtime.js';
...
function _bulletTime() {
  ...
  // Ralentir le moteur Babylon réellement
  const engine = getEngine();
  if (engine) engine.timeScale = CONFIG.combat.bulletTimeScale;
...
function _stopBulletTime() {
  ...
  _c.bulletTimer  = 0;
  const engine = getEngine();
  if (engine) engine.timeScale = 1.0;
...
export function updateCombat(dt) {
  ...
  // Bullet-time timeout
  if (_c.bulletActive) {
    _c.bulletTimer -= dt / CONFIG.combat.bulletTimeScale; // dt réel

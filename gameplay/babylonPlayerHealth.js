...
export function takeDamage(amount, source = 'enemy') {
  if (_isDead) return;
  if (_iFrameTimer > 0) return; // i-frames actives — bloquer les dégâts

  _hp.current = Math.max(0, _hp.current - amount);
  Events.emit('player:damaged', { amount, source, hp: _hp.current });

...
import { initLighting }                        from '../engine/babylon/lighting.js';
...
import { initWorldState, getCurrentAct, setAct } from '../persistence/worldStateManager.js';
...
import { initSky }                             from '../world/babylonSky.js';
...
import { initCombatInput, updateCombat, setCombatEnemySource, getLockedTarget, isInBulletTime, getComboStep } from '../gameplay/babylonCombat.js';
import { initDebugOverlay }                              from './debugOverlay.js';
import { initPlayerHealth, updateHealth, getHp, takeDamage, setIFrames, respawnPlayer, setCurrentHp } from '../gameplay/babylonPlayerHealth.js';
import { spawnEnemy, updateEnemies, getAllEnemies }   from '../gameplay/babylonEnemies.js';
import { initVfx }                             from '../gameplay/babylonVfx.js';
import { initProgression, trackProximity, addItem, gainXp, useItem } from '../gameplay/rpgProgression.js';
...
import { initHud, updateHp, updateStamina, refreshQuest } from '../ui/hud.js';
...
function getState()        { return _gameState; }
...
async function bootstrap() {
  ...
  initProgression(saved?.player ?? null);
  ...
  setCombatEnemySource(useFreePacks ? getAllEnemiesFreePacks : getAllEnemies);
  ...
  Events.on('act:changed', ({ act }) => {
    ...
    if (useFreePacks) {
      const nextBiome = getBiomeForAct(act);
      initPropsFreePacks(scene, nextBiome.name, true).catch((error) => {
        console.warn('[GUTTER GOD] Free-pack props preload failed for biome', nextBiome?.name, error);
      });
    }
  });
  ...
  registerSystem((dt) => {
    ...
    const { stamina } = updateTraversal(dt) ?? {};
  ...
  if (typeof window !== 'undefined') {
    ...
    window.__getAllEnemies    = () => (useFreePacks ? getAllEnemiesFreePacks() : getAllEnemies());
    window.__getTerrainHeight = (x, z) => getTerrainHeight(x, z);
    window.__getConfig      = () => CONFIG;
    window.__getTimeScale   = () => engine?.timeScale ?? 1;
